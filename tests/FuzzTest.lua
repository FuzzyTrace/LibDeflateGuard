...
local function Describe(value)
    if type(value) == "string" then
        return ("<string len=%d>"):format(#value)
    end
...
local function Decode(method, context, ...)
    ...
    if Reference then
        CompareToReference(context, actual,
                           Pack(Reference[method](Reference, ...)))
    end
    return actual
end
...
local function RandomPayload(iteration)
    ...
    if iteration % 3 == 1 then
        return string.rep(RandomString(RandomBetween(1, 12)),
                          math.ceil(length / 6))
    end
...
local function Mutate(str)
    ...
    return str:sub(1, index - 1) .. string.char(Random(256)) ..
               str:sub(index + 1)
end
...
Test("round trips survive every compression level and payload shape", function()
    for iteration = 1, Scaled(120) do
        ...
        for _, case in ipairs({deflate_case, zlib_case}) do
            local compressed = Guard[case.Compress](Guard, payload,
                                                    {level = level})
            local result = Decode(case.Decompress,
                                  case.name .. " round trip", compressed)
            assert(result[1] == payload,
                   case.name .. " round trip did not restore the payload")
            assert(result[2] == 0,
                   case.name .. " round trip status must be 0")
        end
    end
end)
...
Test("truncated, mutated, and trailing members never throw", function()
    for iteration = 1, Scaled(120) do
        ...
        local zcompressed = Guard[zlib_case.Compress](Guard, payload,
                                                      {level = Random(10)})
        for _, pair in ipairs({{deflate_case, compressed},
                               {zlib_case, zcompressed}}) do
            ...
            local result = Decode(case.Decompress, case.name .. " trailing",
                                  extended)
            assert(result[1] == nil,
...
Test("limit policies are enforced and reported", function()
    for iteration = 1, Scaled(120) do
        ...
        local compressed = Guard[deflate_case.Compress](Guard, payload,
                                                        {level = Random(10)})
        for _, case in ipairs({deflate_case, zlib_case}) do
            local member = Guard[case.Compress](Guard, payload,
                                                {level = Random(10)})
            for _ = 1, 4 do
                local limits = {
                    max_input_bytes = RandomBetween(1, #member + 8),
                    max_output_bytes = RandomBetween(1, (#payload > 0 and
                                                        #payload or 1) + 4),
                    max_blocks = RandomBetween(1, 4),
                    max_symbols = RandomBetween(1, (#payload > 1 and #payload or
                                                   2)),
                    max_work_units = RandomBetween(1, (#payload > 1 and
                                                      #payload * 2 or 2))
                }
                local result = Decode(case.Decompress,
                                      case.name .. " tight limits", member,
                                      limits)
                if result[1] then
                    assert(#result[1] <= limits.max_output_bytes,
                           case.name ..
                               " produced more output than max_output_bytes")
                    assert(#member <= limits.max_input_bytes,
                           case.name ..
                               " accepted input over max_input_bytes")
                end
            end

            -- An input one byte over the cap must always be refused.
            local result = Decode(case.Decompress,
                                  case.name .. " input cap", member,
            ...
            if #payload > 1 then
                result = Decode(case.Decompress, case.name .. " output cap",
                                member, {max_output_bytes = #payload - 1})
...
Test("every budget fires when it is set below what the member needs", function()
    ...
    local floors = {
        ...
        {fixed_member, {max_output_bytes = 1},
    ...
    for _, floor in ipairs(floors) do
        ...
        assert(result[2] == expected,
               ("%s = 1 must report %s, got %s"):format(key, expected,
                                                        tostring(result[2])))
    end
    ...
    ok = Decode(deflate_case.Decompress, "lifted budgets multiblock",
                multiblock)
...
Test("invalid policies and argument types are refused, not raised", function()
    ...
    local bad_policies = {
        {max_blocks = 0}, {max_blocks = -1}, {max_blocks = 1.5},
        {unknown_key = 1}, {max_input_bytes = math.huge},
        {max_symbols = "10"}, {max_output_bytes = 0 / 0},
        {max_work_units = -0.5}, "not a table", 42, true
    }
    for _, policy in ipairs(bad_policies) do
        for _, case in ipairs({deflate_case, zlib_case}) do
            local result = Decode(case.Decompress, case.name .. " bad policy",
                                  sample, policy)
            AssertStableFailure(result[1], result[2],
                                case.name .. " bad policy")
    ...
    for _, bad in ipairs({42, true, {}, sample and print}) do
        for _, method in ipairs({"DecompressDeflate", "DecompressZlib",
                                 "DecodeForPrint", "DecodeForWoWAddonChannel",
                                 "DecodeForWoWChatChannel"}) do
...
Test("channel codecs round trip and reject malformed input", function()
    ...
    for _, codec in ipairs(codecs) do
        ...
        for _ = 1, Scaled(1500) do
            ...
            local answer = Decode(DecodeName, name .. " random bytes",
                                  candidate)
...
Test("custom codecs round trip and reject malformed input", function()
    local specs = {
        {"\000", "\001", ""}, {"\000\001\002", "\003", "\004"},
        {"abc", "%", "x"}, {"sS\000\010\013\124%", "\029\031", "\015\020"}
    }
    for _, spec in ipairs(specs) do
        local codec = assert(Guard:CreateCodec(spec[1], spec[2], spec[3]))
        local reference_codec
        if Reference then
            reference_codec = assert(Reference:CreateCodec(spec[1], spec[2],
                                                           spec[3]))
        end
...
Test("dictionary decoding round trips and refuses mismatches", function()
    ...
    for _, bad in ipairs({42, "string", {}, {strlen = 1}}) do
        local result = Decode("DecompressDeflateWithDict", "bad dictionary",
                              sample, bad)
