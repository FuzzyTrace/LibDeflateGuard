...
local valid_vectors = {
    ...
    {
        name = "dynamic",
        compressed = "05c0210d00000080b0fe6d2f916c",
        expected = ""
    }, {
...
Test("input, output, block, symbol, and work limits", function()
    ...
    AssertEqual(decode_error, Guard.ERRORS.WORK_LIMIT_EXCEEDED,
                "work limit error")
end)
...
Test("zlib dictionary use requires an FDICT binding", function()
    ...
    for candidate = 0, 255 do
        if math.floor(candidate / 32) % 2 == 0 and (cmf * 256 + candidate) % 31 ==
            0 then
            flg = candidate
            break
        end
