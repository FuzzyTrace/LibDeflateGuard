...
The audit that found item L, closed out. It read about seventy normative claims
in `README.md` against the module and the suite. Three were false; item L was
the one with a defect behind it and shipped on its own. The other two were
documentation and are fixed here, along with the structural finding, which is
the more useful half: **no test asserted a single number `README.md`
publishes.**

Documentation and tests only. No behaviour changes, `_VERSION` unchanged.

### The recurring failure mode, now five for five

Every false claim items G through L turned up has the same shape: **true in
isolation, false in combination with something else in the same document.** Not
one was a careless sentence. G's preset mutation, H's derived policy, I's
`ERRORS`, K's frozen backstop and L's `__index` policy were each written
correctly against what the author had in view and each contradicted by a
neighbour.

F1 below is the purest specimen so far, because both halves are in the same
`## Performance` section and one of them is three lines away from its own
counterexample. A reviewer reading top to bottom agrees with each paragraph.

The practical lesson is that reading a document for correctness one claim at a
time cannot find this class. What finds it is asking, of each claim, **what
else in this file would have to be true**, and then going to look.

### F1. `## Performance` claimed every figure in it was regenerable

> Every figure in this section comes from `tests/BenchTest.lua`, and can be
> regenerated by anyone. Nothing here is hand-timed.

Two sets of figures sit inside that scope and are neither.

**The four peak-heap figures** — 1.8, 2.3, 14.4 and 22.6 MB, under
`### Worst accepted work per preset`. Three lines above them the same
subsection says peak heap "cannot be sampled from inside Lua", which
`tests/BenchTest.lua` prints for itself at the foot of its own output. So the
section states, in two places a paragraph apart, that these figures are
regenerable and that they cannot be. The paragraph carrying them already calls
them a hand measurement; the scope sentence above simply did not know.

**The paired policy-vs-no-policy comparison** — +0.00%, +1.80%, +0.44% and
"about 0.25 µs of fixed cost per call", under `### Against upstream LibDeflate`. `BindApi` binds one `api.DecompressDeflate` for this module and it
passes the policy on every call; the only environment variables the harness
reads are `LIBDEFLATEGUARD_BENCH_REFERENCE`, `_SEED` and `_ROUNDS`. There is no
knob. Reproducing those percentages means editing the harness to bind a second,
policy-free entry point, so a stranger following the documented commands will
not find them.

**Narrowed, not retracted**, the way item I narrowed the `ERRORS` claim and the
way this section already handles a row inside its own measurement floor. The
figures are useful: one answers "how much does the explicit-policy call shape
cost", which the section raises and would otherwise leave open, and the other
corroborates the allocation totals from a different instrument. Both are now
named in the opening sentence and again where they appear. Everything else in
the section is still one command away, and the phrase the rest of the file
quotes — "nothing else here is hand-timed" — still holds with the exceptions
carved out.

### F1b. `### The compressors` asserted the delta it disclaimed

Flagged by the audit as **not falsifiable as written**, one notch weaker than a
false claim and the same shape. The subsection opens by saying it publishes "no
compressor number of its own and no vs-upstream compressor delta", and closes
with "compression throughput here is upstream's" — which is a vs-upstream
delta, of zero, reached by reading the diff rather than by measuring.

Kept and labelled rather than deleted. It is a useful reading and the reasoning
behind it is sound; what was wrong was presenting it in the register of the
measured figures around it. It now says it is a reading of the code, says
nothing in the harness can confirm or refute it, and bounds the shape of any
error in it: the added work is O(1) per call and outside every loop over the
input, so if the reading is wrong it is wrong by a constant per call rather
than by a factor per byte.

### F2. The `WithPolicy` doc comment claimed a round trip that fails

> Compression is bounded on the bytes going in rather than the bytes coming
> out, because that is what the stall is proportional to and what makes the
> compressed result fit the same policy's decompress input cap on the way back.

The second half is false, and the first half is why: a cap on the bytes going
in says nothing about the bytes coming out, and deflate grows an incompressible
input.

```lua
local guard = LibDeflateGuard.WithPolicy() -- 64 KiB in either direction
local compressed = guard:CompressDeflate(incompressible_65536_bytes)
-- #compressed == 65543
guard:DecompressDeflate(compressed) -- nil, "input_limit_exceeded"
```

`README.md` `## Compression and codecs` states this correctly and has all
along, about 400 lines from the comment that stated the opposite. That distance
is the whole story: neither statement is careless, and nothing forced a reader
of one to meet the other.

Where the comment sits is what makes it worth more than a line edit. It is
directly above the constructor for the object whose round trip —
`guard:DecompressDeflate(guard:CompressDeflate(message))` — this README
recommends twice, in `### Decompress(Compress(x)) is refused on the module` and
again in `### A bound policy instance`. A caller who believed the comment would
believe that idiom is total. It is not, at exactly one input: incompressible
bytes at the boundary. The clause is gone and the counterexample is named in
its place. **No cap and no behaviour changed** — the instance is doing the
right thing, and only the sentence about it was wrong.

### The structural finding, which outlasts the three claims

**No test asserted a single number `README.md` publishes.** Every assertion in
`tests/GuardTest.lua` recomputes the derivation from the module's own tables:
`math.floor(base * 4 / 3)` in "codec decoders cap their input",
`DerivedSymbols(preset.max_input_bytes)` in "the derived backstops track the
policy instead of a constant". Change the 4/3 ratio or the 318 of slack and the
whole suite moves with the module, silently, and only the document is left
holding the old numbers.

The suite's own comment above `DerivedSymbols` names this failure mode —
"a test that asks the module what it derives and then checks it derived that
would pass on any rule" — and then applies it to the formulas and not to the
absolute values, which have exactly the same problem one level down.
`tests/BenchTest.lua` had the right instinct already and checks its literal
preset copy against `LIMIT_PRESETS` before measuring anything; that check is
against the module, not against the document, and it lives in a file CI does
not run as a gate.

### What is pinned

One table of literals near the top of `tests/GuardTest.lua`, transcribed out of
`README.md` with the line and heading each group came from, and six tests over
it. **Transcription is the point.** Read out of the module the whole block
would agree with itself whatever the module became.

- Both `LIMIT_PRESETS` blocks, key for key, plus a key count so a sixth budget
  cannot appear undocumented; `DEFAULT_LIMITS` against the `addon` block, which
  is what "the default" in the document means; `DEFAULT_CODEC_LIMITS` the same
  way.
- The two derivation formulas applied to the **published** budgets, so the
  document's five numbers are checked against the document's own rule rather
  than against the module's.
- The three absolute print caps the document quotes — 87381, 1398101, 262144 —
  reached the way a reader reaches them, by raising `max_input_bytes` on a
  policy and reading back what the instance derived.
- Set equality, both directions, between the values of `ERRORS` and the fifteen
  codes `## Safe decoding` lists as the complete set. One direction catches a
  code added without a doc update; the other catches one the document still
  lists after a rename.
- F2's regression: an incompressible payload at exactly the cap compresses to
  more than the cap and the same instance refuses it, on a payload from a
  Lehmer generator written out in the test. `math.random` is not portable
  between LuaJIT and PUC Lua 5.1, so the same seed would not be the same
  payload — `tests/FuzzTest.lua` carries its own generator for the same reason.
  The inequality is the assertion; 65543 is asserted beside it so a change is
  visible rather than absorbed.
- Every row of `### Arity` through `select("#")`, success and failure, on the
  module and on an instance, with a second pass asserting that the failure rows
  actually failed. An arity table is satisfied by a call that succeeded when it
  was meant to fail. That table is documented as living "here and nowhere
  else", and this is the repository's recurring bug class: v1.1.2 exists
  because an encoder returned a second value into a decoder's cap slot.
- The decompressors' success status being exactly `0`, across a loop that is
  itself asserted to have reached `padding_bitlen` 4, 5 and 6 — otherwise the
  claim is `0 == 0` over a padding that was always zero.
- `DecodeForPrint`'s strip of leading and trailing control characters and
  spaces, including what it does not forgive: an interior space at unchanged
  length, and the input cap applying to the string as passed. The audit found
  this documented nowhere, in a fork whose headline is strict rejection. It is
  inherited from upstream and it is deliberate — a print string is a thing
  users paste — so it is now written down in `## Compression and codecs` **and**
  pinned. A later tightening should be a decision, not a silent break for every
  caller pasting a string with a newline on it.

### What is deliberately not pinned, and why

Worth as much as the list above, because an untestable claim tested badly is
worse than one left alone.

- **The resident-memory table.** The header read "LibDeflateGuard 1.3.0" in a
  1.3.1 document and the fork's row had drifted about a kilobyte. Both fixed;
  no test. A resident figure fails on every release that adds a line, which
  makes it a change detector rather than a claim — the defect, not the fix. The
  finding that table carries is the ×10 multiplier against a LibStub-shared
  upstream, and no plausible drift touches it. Recorded here: re-taking it
  reproduced the **upstream** cells unchanged, which is what licensed updating
  the fork's; a reimplementation of the stated protocol that had disagreed with
  upstream would have been measuring something else. The neighbouring
  first-copy premium is a different metric, was not reproduced, and was left
  alone rather than half-updated. A KiB/MiB slip in the same paragraph — ten
  copies of 175 KiB called "about 1.75 MiB" — is also fixed, and is the fourth
  inaccuracy this audit turned up.
- **Anything in `## Performance`.** Timings are machine-dependent and the
  document says so twice; a test asserting one would fail on the next machine.
  What is testable there is now testable: the scope sentence names its
  exceptions, so a reader can check the claim by running the harness rather
  than by trusting the sentence.
- **The four unfalsifiable claims** the audit flagged alongside the three false
  ones. F1b is fixed above because it could be made falsifiable by saying what
  it is. The other three cannot be, and are left standing with that recorded:
  the "about 1.9 MB/s at level 6" compression figure, which is a hand
  measurement on a named machine and is quoted where it is used rather than
  repeated; the claim that the World of Warcraft client's interpreter is slower
  than the machine `### One machine` names, which nothing in this repository
  can run against; and the resident-memory protocol's "does not move when more
  collection cycles are added", which is a property of a measurement session
  rather than of the library. Each is honest, none is checkable from a test,
  and inventing a test for any of them would pin the harness rather than the
  claim.
- **`tests/ClaimsTest.lua`.** Considered and rejected. `GuardTest` is already
  the claims file in all but name. A third dependency-free suite would need its
  own `AssertEqual` and `FromHex`, a third CI invocation and a third paragraph
  in `## Validation` — new documented surface that can go stale, in an audit
  whose entire subject is documented surface going stale.

### Regression test and gate

Checked by mutation, one claim at a time, restoring from a byte copy between
runs. Each of these fails the new test that names it: the 4/3 print ratio to
5/4, the 318 of slack to 319, the 336 per block to 337, `addon`'s `max_blocks`
from 256 to 512, `ERRORS.INVALID_PRINT` renamed, the decompress success status
replaced by a padding-like number, and `codec:Encode` returning `string_gsub`'s
count again — the v1.1.2 shape.

Two of those are the point of the exercise. **`max_blocks` 256 → 512 fails one
test in the suite and it is the new one**: every other assertion reads the
budget out of the module and moves with it. The renamed `ERRORS` value is
caught in both directions by the set-equality test, where before it would have
been caught only where some other assertion happened to name that code.

`tests/BenchTest.lua`, PR #15 and its branch are untouched.

The differential gate is clean against v1.3.1: 13,246 compared calls at the
default iteration count and 152,405 at `LIBDEFLATEGUARD_FUZZ_ITERATIONS=12`,
zero divergences. It has to be — nothing outside a comment changed in
`LibDeflateGuard.lua` — and it is run and recorded anyway, because "this one is
only documentation" is how a behaviour change gets in without a gate.
