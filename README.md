...
before it decodes anything, which is inherited from upstream and is the one
place this fork is lenient rather than strict. `" " .. encoded .. "\n"`
decodes to the same bytes as `encoded`, and so does a copy that picked up a tab
or a carriage return at either end. That is what the word "stripped" in the
paragraph above refers to. It is deliberate — a print-codec string is a thing
users paste, and a pasted string arrives with whitespace on it — and it is
narrow: only the two ends, only `%c` and the space character, and the length
and canonical-encoding checks then run against what is left, so nothing about
the interior of the string is forgiven. The input cap is applied to the string
you passed, before the strip, so padding counts against it. The channel
decoders and `codec:Decode` strip nothing.
...
Every timing and allocation figure in this section comes from
`tests/BenchTest.lua` and can be regenerated by anyone, with two exceptions,
both marked where they appear:

- the four **peak-heap** figures under `### Worst accepted work per preset`,
  which are hand measurements because peak heap cannot be sampled from inside
  Lua — the harness says so itself, and the allocation totals beside them are
  the regenerable quantity;
- the **paired policy-vs-no-policy** comparison under
  `### Against upstream LibDeflate`, which the shipped harness does not
  produce: `BindApi` passes the policy on every one of this module's
  decompress calls and has no knob to drop it, so those three percentages came
  from an edited harness.

Nothing else here is hand-timed, and nothing else needs anything but the two
commands under `### One machine`. The exceptions are stated rather than dropped
because both answer a question the regenerable figures raise and neither can be
made regenerable — one by the interpreter, one by the harness's own call shape.
...
**These three percentages are the exception this section opens with.** The
shipped harness cannot produce them: `BindApi` binds one `api.DecompressDeflate`
that passes the policy on every call of this module's, and there is no
environment variable or argument that drops it. Reproducing +0.00%, +1.80% and
+0.44% means editing `tests/BenchTest.lua` to bind a second, policy-free entry
point and alternating against it. They are reported anyway, because the caveat
above is better answered with a measurement than left open, and they are marked
because a reader who runs the harness as documented will not find them.
...
number of its own and no _measured_ vs-upstream compressor delta. The one
compression figure this file quotes — about 1.9 MB/s at level 6, under
`## Compression and codecs` — is an earlier hand measurement, kept where it is
used rather than repeated here, so that "nothing else here is hand-timed" stays
true of this section.
...
compression work begins and none of it is inside a loop over the input.

From which it follows, **on inspection of the diff rather than from a
measurement**, that compression throughput here should be upstream's. That is
the one vs-upstream compressor statement in this section and it is unmeasured;
nothing in `tests/BenchTest.lua` can confirm or refute it, which is why it is
worded as a reading of the code and not as a result. What the reading does
bound is the shape of any error in it: the added work is a fixed cost per call
and sits outside every loop over the input, so if it is wrong it is wrong by a
constant per call, not by a factor per byte. Anyone who needs the number
measured should measure it; this file does not.
...
interesting. This fork costs about 20 KiB more per copy than upstream on either
interpreter — 13% on Lua 5.1, 19% under LuaJIT — which is the guard machinery
itself. The multiplier is the finding. Ten addons embedding LibDeflateGuard
hold about 1.7 MiB of module on a shared Lua 5.1 heap where addon memory is a
...
The fork's two cells were re-taken at 1.3.1 and each moved up about a
kilobyte; the upstream cells came back unchanged to the precision printed,
which is the check that the second reading was the same reading. Expect this
column to drift by a kilobyte or two per release. Nothing in the suite pins it,
deliberately: a test asserting a resident figure fails on every release that
adds a line, which makes it a change detector rather than a claim, and the
finding here is the ×10 multiplier rather than any one cell.
