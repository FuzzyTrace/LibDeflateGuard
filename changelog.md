...
- **Documentation and tests.** A claims audit of `README.md` against the
  module found two remaining false statements and one structural gap in the
  suite. No behaviour changes; `_VERSION` is unchanged.

  **The `## Performance` scope sentence was false.** It opened "Every figure in
  this section comes from `tests/BenchTest.lua`, and can be regenerated by
  anyone", and two sets of figures inside its own scope cannot be. The four
  peak-heap figures are hand measurements — the same subsection says peak heap
  cannot be sampled from inside Lua, so the section asserted that they both are
  and are not regenerable — and the paired policy-vs-no-policy percentages
  needed an edited harness, because `BindApi` passes the policy on every one of
  this module's decompress calls and has no knob to drop it. The claim is
  narrowed rather than retracted: the figures stay, the two exceptions are
  named in the opening sentence and again where they appear, and everything
  else is still one command away.

  **`### The compressors` asserted the delta it had just disclaimed.** The
  section says it publishes no vs-upstream compressor delta and then closes
  with one — "compression throughput here is upstream's" — reached by reading
  the diff. It now says so: the statement is kept, labelled as a reading of the
  code rather than a measurement, and bounded, since the added work is a fixed
  cost per call outside every loop over the input.

  **The `WithPolicy` doc comment claimed a round trip that does not hold.** It
  said the derived compression cap is "what makes the compressed result fit the
  same policy's decompress input cap on the way back". It is not:
  ...
  `## Compression and codecs` in `README.md` has always said this correctly,
  400 lines away from the comment that said the opposite. The false clause is
  gone and the comment now names the counterexample, because it sits directly
  above the round-trip idiom this file recommends twice.

  **`DecodeForPrint` strips leading and trailing control characters and
  spaces**, so `" " .. encoded .. "\n"` decodes. That is inherited from
  upstream, is deliberate — a print string is a thing users paste — and was
  documented nowhere in a fork whose headline is strict rejection. It is now in
  `## Compression and codecs`, with what it does not forgive: only the two
  ends, and the input cap still applies to the string as passed.

  **The resident-memory table was labelled 1.3.0** in a 1.3.1 document. The
  label is fixed and the fork's two cells re-taken; the upstream cells came
  back unchanged, which is the check that the second reading was the same
  reading.

  **Tests.** No test asserted a single number `README.md` publishes — every one
  recomputed the derivation from this module's own tables, so changing a
  constant moved the whole suite and left only the document holding the old
  numbers. `tests/GuardTest.lua` gains six tests built on one table of literals
  transcribed from the document rather than read out of the module: the two
  `LIMIT_PRESETS` blocks and `DEFAULT_CODEC_LIMITS` key for key, the three
  absolute print caps, both derivation formulas applied to the published
  budgets, set equality between `ERRORS` and the fifteen documented codes, the
  compress-past-the-cap regression above on a seeded payload, `select("#")` on
  every row of `### Arity` on both the module and an instance, the success
  status being exactly `0` across the compressor's whole `padding_bitlen`
  range, and the `DecodeForPrint` strip. 33 tests to 39.
