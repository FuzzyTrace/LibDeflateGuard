...
local compress_deflate = LibDeflateGuard:CompressDeflate(example_input)

-- decompress
local decompress_deflate, decode_error =
  LibDeflateGuard:DecompressDeflate(compress_deflate)
-- Check if the first return value of DecompressXXXX is non-nil to know if the
-- decompression succeeds.
if decompress_deflate == nil then
  error("Decompression failed: " .. decode_error)
else
  -- Decompression succeeds.
...
local data_to_trasmit_WoW_addon = LibDeflateGuard:EncodeForWoWAddonChannel(
                                    compress_deflate)
-- When the receiver gets the data, decoded it first.
local data_decoded_WoW_addon = LibDeflateGuard:DecodeForWoWAddonChannel(
                                 data_to_trasmit_WoW_addon)
-- Then decomrpess it
assert(LibDeflateGuard:DecompressDeflate(data_decoded_WoW_addon) ==
         example_input)
...
local printable_compressed = LibDeflateGuard:EncodeForPrint(compress_deflate)
...
assert(LibDeflateGuard:DecodeForPrint(printable_compressed) == compress_deflate)
...
local compress_zlib = LibDeflateGuard:CompressZlib(example_input)
local decompress_zlib = LibDeflateGuard:DecompressZlib(compress_zlib)
...
local compress_deflate_with_level = LibDeflateGuard:CompressDeflate(
                                      example_input, {level = 9})
local decompress_deflate_with_level = LibDeflateGuard:DecompressDeflate(
...
-- print(LibDeflateGuard:Adler32(dict_str), #dict_str)
-- 9 147325380
...
-- internet.
local dict = LibDeflateGuard:CreateDictionary(dict_str, 9, 147325380)

-- Using the dictionary with raw deflate format
local compress_deflate_with_dict = LibDeflateGuard:CompressDeflateWithDict(
                                     example_input, dict)
assert(#compress_deflate_with_dict < #compress_deflate)
local decompress_deflate_with_dict = LibDeflateGuard:DecompressDeflateWithDict(
...
local compress_zlib_with_dict = LibDeflateGuard:CompressZlibWithDict(
